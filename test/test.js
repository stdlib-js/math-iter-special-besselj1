...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterBesselj1, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterBesselj1( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the Bessel function of the first kind of order one for each iterated value', function test( t ) {
	var expected;
	var it;
	var N;
	var x;
	...
	N = 101;
	it = iterBesselj1( linspace( -2.0, 2.0, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( -2.0, 2.0, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = besselj1( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the Bessel function of the first kind of order one for each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': besselj1( values[ 0 ] ),
			'done': false
		},
		{
			'value': besselj1( values[ 1 ] ),
			'done': false
		},
		{
			'value': besselj1( values[ 2 ] ),
			'done': false
		},
		{
			'value': besselj1( values[ 3 ] ),
			'done': true
		}
	];
